...
class MySQLDriver {
  ...
  async getRecordsCount(
    ...
    let self = this;
    return await self._selectRecordRawCount(
      table_name,
      where,
      order_by,
      options
  ...
  async updateRecords(table_name: string, properties: any, where: any) {
    let self = this;
    let { database } = self.config;
    let clean_properties = await self._prepareRecord(
      database,
      table_name,
      properties
    );
  ...
  async _prepareRecord(
    ...
    if (!(typeof table_name === 'string')) {
      let error: any = new Error(
        `MySQLDriver in function _prepareRecord: Provided table name is not a string.`
      );
  ...
  async _selectRecordRaw(
    ...
    const { sql, params, isResultEmpty } = _prepareSelectStatement(
      table_name,
      where,
      order_by,
      options
 ...
 * @param where
 */
function _prepareSelectStatement(
  table_name: string,
  where: any = {},
  order_by: Array<{ key: string; order: 'ASC' | 'DESC' }>,
  options?: QueryOptions
