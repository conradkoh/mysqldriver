...
describe('All Tests', () => {
    ...
    it('Get Records', async () => {
        ...
        let record = await db.getRecords('user', { user_id: user.user_id, index_number: user.index_number });
        chai.assert(record.length > 0, 'Failed to get records.');
    });
    it('Get Records Limit', async () => {
        let record = await db.getRecords('user', {}, null, { limit: { offset: 0, page_size: 1 } });
        chai.assert(record.length === 1, 'Failed to limit records.');
    });
    it('Get Records Order', async () => {
        let user = Object.values(users)[1];
        let records = await db.getRecords('user', {}, [{ key: 'index_number', order: 'DESC' }], { limit: { offset: 0, page_size: 1 } });
    ...
    it('Get Records Order Wildcard (before)', async () => {
        ...
        let records = await db.getRecords('user', { user_id: user_id_partial }, [{ key: 'index_number', order: 'DESC' }], {
    ...
    it('Get Records Count Limit', async () => {
        let count = await db.getRecordsCount('user', {}, null, { limit: { offset: 0, page_size: 1 } });
        chai.assert(count === 1, 'Failed to limit records.');
    });
    it('Get Records Count Order', async () => {
        let count = await db.getRecordsCount('user', {}, [{ key: 'index_number', order: 'DESC' }], { limit: { offset: 0, page_size: 1 } });
        chai.assert(count === 1, 'Failed to limit records.');
    });
    ...
    it('Update Records', async () => {
        ...
        await db.updateRecords('user', TEST_DATA.USER_1_UPDATED, { user_id: user1.user_id });
        user1 = {
            ...user1,
        ...
        chai.assert.equal(user1_updated.first_name, TEST_DATA.USER_1_UPDATED.first_name, 'Failed to up update record.');
        let user2 = Object.values(users)[1];
        let user2_record = await db.getRecord('user', { user_id: user2.user_id });
        for (let key in user2) {
            chai.assert.strictEqual(user2[key], user2_record[key], 'Unexpected record update.');
        }
    });
