...
const DB = Reference.system["database.js"].require();
const FS = require('fs-extra');
const PATH = require('path');
const out_folder = PATH.resolve(__dirname + '/../references/db_objects');
async function main() {
    const schema = await DB.getJSSchema();
    for (var table_schema of schema) {
        //Check if the file requires updating before generating new model
        const TABLE_NAME = table_schema.table_name;
        const MODEL_FILE = getOutputFilePath(TABLE_NAME, out_folder);
        try {
            const ModelClass = require(MODEL_FILE);
            let model = new ModelClass({});
            const DB_FIELDS = await DB.getTableFieldNames(TABLE_NAME);
            model.validate(DB_FIELDS);
            // updateDBModel(table_schema, OUT_FOLDER); //Force model to generate
            console.log(`[ok] - ${TABLE_NAME}`)
        } catch (err) {
            FS.removeSync(MODEL_FILE);
            updateDBModel(table_schema, out_folder);
        }
    }
}

function updateDBModel(table_schema, outfolder) {
